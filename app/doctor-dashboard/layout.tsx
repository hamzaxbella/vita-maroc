...
import { FiHome, FiUsers, FiCalendar, FiMapPin, FiClock, FiSettings, FiMenu, FiX, FiLogOut } from 'react-icons/fi';
...
export default function DoctorDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (user.userType !== 'doctor') {
        // If not a doctor, redirect to appropriate dashboard or login
        router.push('/auth');
        return;
      }
      ...
      // Check if doctor is verified and the current page is not the verification page
      // This allows unverified doctors to only access the verification page
      const usersJson = localStorage.getItem('vita_users');
      if (usersJson) {
        const users = JSON.parse(usersJson);
        const currentDoctor = users.find((u: any) => u.email === user.email && u.userType === 'doctor');
        
        if (currentDoctor && !currentDoctor.isVerified && !pathname.includes('/verification')) {
          // If doctor is not verified and not on verification page, redirect to verification
  ...
  const navItems = [
    { name: 'Home', href: '/doctor-dashboard', icon: <FiHome size={20} /> },
    { name: 'My Patients', href: '/doctor-dashboard/my-patients', icon: <FiUsers size={20} /> },
    { name: 'Appointments', href: '/doctor-dashboard/appointments', icon: <FiCalendar size={20} /> },
    { name: 'Home Visits', href: '/doctor-dashboard/home-visits', icon: <FiMapPin size={20} /> },
    { name: 'Availability Settings', href: '/doctor-dashboard/availability-settings', icon: <FiClock size={20} /> },
    { name: 'Profile Settings', href: '/doctor-dashboard/profile', icon: <FiSettings size={20} /> },
  ];
